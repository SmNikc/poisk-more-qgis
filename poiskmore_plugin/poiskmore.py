...
from PyQt5.QtWidgets import QPushButton, QWidget # Добавлены для RegionDialog fallback
...
class PoiskMorePlugin:
    ...
    def unload(self):
        for action in self.actions:
            # Проверяем, что action все еще привязан к меню, прежде чем пытаться его удалить
            # Это предотвращает ошибки, если меню уже было удалено или action отвязан
            if self.menu.actions().__contains__(action):
                self.menu.removeAction(action)
        # Удаляем само меню из главного окна QGIS
        if self.menu:
            self.iface.mainWindow().menuBar().removeAction(self.menu.menuAction())
            self.menu.deleteLater() # Освобождаем ресурсы меню
        self.actions.clear() # Очищаем список действий
    def run_sitrep(self):
