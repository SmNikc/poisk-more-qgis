...
from .interface_simplifier import InterfaceSimplifier, add_interface_controls_to_menu
...
class PoiskMorePlugin:
    ...
    def initGui(self):
        ...
        # Добавляем управление интерфейсом в меню Сервис
        self.interface_simplifier = add_interface_controls_to_menu(self.menu_manager)
        ...
        try:
            self._pm_sidebar = ensure_pm_sidebar_dock(self.iface)
        ...
        try:
            ...
            proj_manager.auto_setup()
            QgsApplication.messageLog().logMessage("Проекции настроены автоматически", "Poisk-More", Qgis.Info)
        except Exception as e:
            QgsApplication.messageLog().logMessage(f"Projection manager error: {e}", "Poisk-More", Qgis.Warning)
