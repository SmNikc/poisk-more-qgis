...
class DBManager:
    """Простое управление SQLite-базой для плагина."""
    ...
    def save_incident(self, type_: str, lat: float, lon: float, description: str) -> None:
        cursor = self.conn.cursor()
        cursor.execute(
            "INSERT INTO incidents (type, lat, lon, description) VALUES (?, ?, ?, ?)",
            (type_, lat, lon, description),
        )
        self.conn.commit()

    def save_sitrep(self, type_: str, datetime: str, sru: str, zone: str, notes: str) -> None:
        cursor = self.conn.cursor()
        cursor.execute(
            "INSERT INTO sitrep (type, datetime, sru, zone, notes) VALUES (?, ?, ?, ?, ?)",
            (type_, datetime, sru, zone, notes),
        )
        self.conn.commit()
