...
def initGui(self):
if not self.iface:
print("Ошибка: iface не инициализирован") return
self.menu = QMenu("Поиск-Море", self.iface.mainWindow().menuBar()) self.iface.mainWindow().menuBar().addMenu(self.menu) self._add("SITREP", self.run_sitrep)
self._add("Район поиска", self.run_region) self._add("Учение", self.run_exercise) self._add("Инцидент", self.run_err) self._add("Вероятность", self.run_probability) self._add("Маршрут SRU", self.run_route) self._add("Схема поиска", self.run_scheme) self._add("Лог", self.run_log)
...
def unload(self):
for action in self.actions: self.iface.removePluginMenu("Поиск-Море", action)
