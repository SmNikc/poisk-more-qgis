...
def generate_html(root_dir, output_html_path, title="Содержимое проекта"):
    ...
    # Получим абсолютный путь к папке вывода, чтобы исключить её из обхода
    ...
    with open(output_html_path, 'w', encoding='utf-8') as f:
        ...
        for dirpath, dirnames, filenames in os.walk(root_dir):
            ...
            # Пропуск папки вывода, чтобы избежать рекурсии
            if abs_dirpath.startswith(output_dir):
                print(f"Пропуск директории вывода: {abs_dirpath}")
                dirnames.clear()  # Не спускаться в поддиректории
                continue
            ...
            for filename in filenames:
                ...
                # Вывод прогресса
                print(f"Обработка файла: {rel_file_path}")
